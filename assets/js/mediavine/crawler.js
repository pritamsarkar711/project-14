/* huvanti Mediavine checker — browser crawler (independent of AdSense & Ezoic). */
(function (global) {
  'use strict';
  var MV = global.MediavineChecker = global.MediavineChecker || {};
  ...
  function sameSite(a, b) { try { return normHost(new URL(a).hostname) === normHost(new URL(b).hostname); } catch (e) { return false; } }
  ...
  function headersToObj(h) { var o = {}; if (!h) return o; if (h.forEach) h.forEach(function (v, k) { o[k.toLowerCase()] = v; }); return o; }
  ...
  function fetchOnce(rawUrl, signal) {
    ...
    function ok(res, text, via, extra) {
      ...
      return { url: rawUrl, finalUrl: extra.finalUrl || res.url || rawUrl, status: extra.status != null ? extra.status : (res.status || 200), ok: (extra.status != null ? extra.status : res.status) < 400, redirected: !!res.redirected, headers: extra.headers || headersToObj(res.headers), text: text || '', via: via, ms: extra.ms || null, bytes: (text || '').length };
    }
    return fetch(rawUrl, { redirect: 'follow', signal: ctrl.signal, headers: accept }).then(function (res) {
      ...
      return fetch('https://api.allorigins.win/get?url=' + encodeURIComponent(rawUrl), { signal: ctrl.signal }).then(function (res) { return res.json(); }).then(function (j) {
        var h = {};
        if (j.status) { if (j.status.content_type) h['content-type'] = j.status.content_type; if (j.status.http_code) h[':http'] = String(j.status.http_code); }
        return ok({ url: rawUrl, redirected: false, headers: { forEach: function () {} } }, j.contents || '', 'allorigins', { finalUrl: (j.status && j.status.url) || rawUrl, status: (j.status && j.status.http_code) || 200, headers: h });
      }).catch(function () {
        if (signal && signal.aborted) { clearTimeout(to); throw makeError('cancelled', 'Audit cancelled.'); }
        return fetch('https://corsproxy.io/?url=' + encodeURIComponent(rawUrl), { signal: ctrl.signal }).then(function (res) { return res.text().then(function (text) { return ok(res, text, 'corsproxy'); }); }).catch(function () {
          if (signal && signal.aborted) { clearTimeout(to); throw makeError('cancelled', 'Audit cancelled.'); }
          return fetch('https://api.codetabs.com/v1/proxy/?quest=' + encodeURIComponent(rawUrl), { signal: ctrl.signal }).then(function (res) { return res.text().then(function (text) { if (/^[A-Za-z ]+Error/i.test(text.slice(0, 80))) throw new Error(text.slice(0, 80)); return ok(res, text, 'codetabs'); }); }).catch(function (err) {
            clearTimeout(to);
  ...
  var PRIORITY = [
    ...
    [/privacy/i, 98], [/terms|conditions/i, 97], [/disclaimer/i, 96], [/cookie/i, 95], [/editorial/i, 94],
  ...
  function Crawler(startUrl, opt) {
    ...
    this.sitemapUrls = [];
    this.adsTxt = { present: false, text: '', hasMediavine: false, lineCount: 0 };
  ...
  Crawler.prototype.crawlOne = function (item) {
    var self = this;
    return MV.fetchText(item.url, { signal: self.signal, retries: 1 }).then(function (info) {
      ...
      self.results.push({ url: item.url, finalUrl: info.finalUrl || item.url, status: info.status, depth: item.depth, redirected: info.redirected, via: info.via, ms: info.ms, bytes: info.bytes, headers: info.headers, html: info.text, challenge: self.challenge });
  ...
  Crawler.prototype.run = function () {
    ...
    this.onProgress({ stage: 'connect', message: 'Connecting…' });
    seq = seq.then(function () { return MV.fetchText(self.origin + '/robots.txt', { signal: self.signal, retries: 0 }).then(function (r) { if (r.status < 400 && r.text) self.robots = MV.parseRobots(r.text); }).catch(function () { self.robots = MV.parseRobots(''); }); });
    seq = seq.then(function () { return MV.fetchText(self.origin + '/ads.txt', { signal: self.signal, retries: 0 }).then(function (r) { if (r.status < 400 && r.text && !/^\s*<(!doctype|html)/i.test(r.text)) { self.adsTxt = { present: true, text: r.text.slice(0, 20000), hasMediavine: /mediavine\.com/i.test(r.text), lineCount: r.text.split(/\r?\n/).filter(function (l) { return l.trim() && l.trim().charAt(0) !== '#'; }).length }; } }).catch(function () {}); });
    seq = seq.then(function () {
      self.onProgress({ stage: 'sitemap', message: 'Discovering sitemap…' });
      var cands = (self.robots.sitemaps || []).concat([self.origin + '/sitemap.xml', self.origin + '/sitemap_index.xml', self.origin + '/wp-sitemap.xml']);
      var seen = new Set();
      var seenU = new Set();
      var collect = [];
      var promises = cands.filter(function (u) { var n = normalizeUrl(u, self.origin); if (!n || seen.has(n)) return false; seen.add(n); return true; }).slice(0, 6).map(function (loc) {
        return MV.fetchText(loc, { signal: self.signal, retries: 0 }).then(function (r) {
          var re = /<loc>\s*([^<]+)\s*<\/loc>/gi, m;
          while ((m = re.exec(r.text || ''))) { var u = m[1].trim().replace(/&amp;/g, '&'); var n = normalizeUrl(u, self.origin); if (n && sameSite(n, self.origin) && !ASSET.test(n) && !seenU.has(n)) { seenU.add(n); collect.push(n); } }
        }).catch(function () {});
      });
      return Promise.all(promises).then(function () { self.sitemapUrls = collect; });
    });
    return seq.then(function () {
      self.enqueue(self.start, 0);
      (self.sitemapUrls || []).slice(0, self.limit * 2).forEach(function (u) { self.enqueue(u, 1); });
      self.onProgress({ stage: 'crawler', message: 'Crawling…', crawled: 0, limit: self.limit });
      function loop() {
        if (self.visited.size >= self.limit) return Promise.resolve();
        if (self.signal && self.signal.aborted) return Promise.reject(makeError('cancelled', 'Audit cancelled.'));
        self.queue.sort(function (a, b) { return (b.priority - a.priority) || (a.depth - b.depth); });
        var batch = [];
        while (batch.length < self.concurrency && self.queue.length && self.visited.size + batch.length < self.limit) { var it = self.queue.shift(); if (self.visited.has(it.url)) continue; self.visited.add(it.url); batch.push(it); }
        if (!batch.length) return Promise.resolve();
        return Promise.all(batch.map(function (it) { return self.crawlOne(it); })).then(function () { self.onProgress({ stage: 'crawler', message: 'Crawled ' + self.visited.size + ' / ' + self.limit + ' pages…', crawled: self.visited.size, limit: self.limit }); return loop(); });
      }
      return loop();
    }).then(function () {
      return { start: self.start, origin: self.origin, limit: self.limit, robots: self.robots, sitemapUrls: self.sitemapUrls, adsTxt: self.adsTxt, pages: self.results, errors: self.errors, challenge: self.challenge, reachedLimit: self.queue.length > 0 || self.visited.size >= self.limit };
    });
  };
