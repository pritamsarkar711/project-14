...
(function (global) {
  ...
  function Crawler(startUrl, opt) {
    ...
    this.sitemapUrls = [];
    this.adsTxt = { present: false, text: '', hasRaptive: false, lineCount: 0 };
  ...
  Crawler.prototype.run = function () {
    ...
    seq = seq.then(function () { return MV.fetchText(self.origin + '/ads.txt', { signal: self.signal, retries: 0 }).then(function (r) { if (r.status < 400 && r.text && !/^\s*<(!doctype|html)/i.test(r.text)) { self.adsTxt = { present: true, text: r.text.slice(0, 20000), hasRaptive: /raptive\.com/i.test(r.text), lineCount: r.text.split(/\r?\n/).filter(function (l) { return l.trim() && l.trim().charAt(0) !== '#'; }).length }; } }).catch(function () {}); });
