...
- **Measurement transport**: the page is loaded into a sandboxed same-origin iframe (localStorage/sessionStorage/cookies/service workers isolated; top-navigation blocked). Preferred transport is the server proxy (`/api/cwv-proxy`, DNS-pinned, redirects re-validated, size-capped, optional network throttling) which also records response headers for the caching/compression audits. When the server has no direct outbound access (e.g. this preview sandbox), the visitor's browser fetches the page through public read-only relays and the same measurement runs browser-direct (cross-origin timing holes are marked unavailable, never estimated).
