...
const { analyzeLongForm } = require('./longFormAnalyzer');
const { analyzeHumanInvolvement } = require('./humanInvolvementAnalyzer');
const { analyzeBrandSafety } = require('./brandSafetyAnalyzer');
const { analyzeTrafficSignals } = require('./trafficAnalyzer');
const { analyzeUX } = require('./uxAnalyzer');
const { analyzeAdvertising } = require('./adReadinessAnalyzer');
const { analyzeTechnical } = require('./technicalAnalyzer');
const { analyzeTrust } = require('./trustAnalyzer');
const { analyzeAnalytics } = require('./analyticsAnalyzer');
const { lookupDomainAge, analyzeDomain } = require('./domainAnalyzer');
...
function analyzeParsed(scan, opt) {
  ...
  onProgress({ stage: 'important', message: 'Page classification completed…' });
  ...
  const ctx = {
    ...
    user: opt.user || scan.user || {},
    domainAge: scan.domainAge || { verified: false }
  };

  onProgress({ stage: 'content', message: 'Content analysis completed…' });
  let findings = [];
  findings = findings.concat(analyzeContent(parsed, ctx));
  onProgress({ stage: 'duplicates', message: 'Duplicate analysis completed…' });
  findings = findings.concat(analyzeDuplicates(parsed, ctx));
  findings = findings.concat(analyzeOriginality(parsed, ctx));
  onProgress({ stage: 'longform', message: 'Long-form analysis completed…' });
  findings = findings.concat(analyzeLongForm(parsed, ctx));
  findings = findings.concat(analyzeHumanInvolvement(parsed, ctx));
  ctx.inventory = inventoryFrom(parsed, ctx);
  onProgress({ stage: 'technical', message: 'Technical audit completed…' });
  findings = findings.concat(analyzeTechnical(pages, ctx));
  onProgress({ stage: 'ux', message: 'Reader-experience analysis completed…' });
  findings = findings.concat(analyzeUX(parsed, ctx));
  ctx.contentPortfolio = ctx.contentPortfolio || {};
  onProgress({ stage: 'trust', message: 'Trust & transparency analyzed…' });
  findings = findings.concat(analyzeTrust(parsed, ctx));
  onProgress({ stage: 'advertising', message: 'Ad-readiness analysis completed…' });
  findings = findings.concat(analyzeAdvertising(parsed, ctx));
  onProgress({ stage: 'brand', message: 'Brand-safety analysis completed…' });
  findings = findings.concat(analyzeBrandSafety(parsed, ctx));
  onProgress({ stage: 'traffic', message: 'Traffic-readiness analysis completed…' });
  findings = findings.concat(analyzeTrafficSignals(parsed, ctx));
  onProgress({ stage: 'analytics', message: 'Google Analytics detection completed…' });
  findings = findings.concat(analyzeAnalytics(parsed, ctx));
  findings = findings.concat(analyzeDomain(ctx));
  onProgress({ stage: 'requirements', message: 'Official requirement verification…' });
  const req = evaluateOfficial(parsed, ctx);
  findings = findings.concat(req.findings);

  const conf = crawlConfidence(scan, parsed, ctx);
  onProgress({ stage: 'score', message: 'Calculating Raptive readiness…' });
...
async function runAudit(rawUrl, opt) {
  ...
  const crawler = new Crawler(rawUrl, { limit: opt.limit || 50, concurrency: opt.concurrency || 4, signal: opt.signal, onProgress });
  const scan = await crawler.run();
  onProgress({ stage: 'domain', message: 'Looking up domain age…' });
  try {
    const host = new URL(scan.origin || scan.start).hostname;
    scan.domainAge = await lookupDomainAge(host, { signal: opt.signal });
  } catch (e) {
    scan.domainAge = { verified: false, reason: 'Domain age lookup failed.' };
  }
  ...
  return analyzeParsed(scan, { onProgress, user: opt.user || {} });
}
