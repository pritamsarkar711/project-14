...
function analyzeTechnical(pages, ctx) {
  ...
  out.push(R.finding(R.get('RAP-H-TECH'), 'Site', https ? 'passed' : 'high',
  ...
  if (ctx.robots && ctx.robots.blocksAll) {
    out.push(R.finding(R.get('RAP-H-TECH'), 'Site', 'critical',
    ...
    out.push(R.finding(R.get('RAP-H-TECH'), 'Site', 'passed',
    ...
    out.push(R.finding(R.get('RAP-H-TECH'), 'Site', 'low',
      'No robots.txt found.',
      { confidence: 0.8, severity: 'low' }));
  }

  out.push(R.finding(R.get('RAP-H-TECH'), 'Site', (ctx.sitemapUrls && ctx.sitemapUrls.length) ? 'passed' : 'low',
  ...
  if (ctx.challenge) {
    out.push(R.finding(R.get('RAP-H-TECH'), 'Site', 'high',
  ...
  pages.forEach(p => {
    ...
    if (p.error) {
      out.push(R.finding(R.get('RAP-H-TECH'), path, 'high',
        'Could not read ' + path + ': ' + p.error,
        { confidence: 0.7, severity: 'high' }));
      return;
    }
    if (p.status >= 500) { serverErr++; out.push(R.finding(R.get('RAP-H-TECH'), path, 'high', 'HTTP ' + p.status + '.', { confidence: 0.95, severity: 'high' })); }
    else if (p.status >= 400) { notFound++; out.push(R.finding(R.get('RAP-H-TECH'), path, 'medium', 'HTTP ' + p.status + '.', { confidence: 0.95, severity: 'medium' })); }
    ...
    if (pa.noindex) {
      noindex++;
      out.push(R.finding(R.get('RAP-H-TECH'), path, path === '/' ? 'critical' : 'medium',
    ...
    if (!pa.title) out.push(R.finding(R.get('RAP-H-TECH'), path, 'high', 'No <title> tag.', { confidence: 0.92, severity: 'high' }));
    else if (pa.titleLen < 12 || pa.titleLen > 70) out.push(R.finding(R.get('RAP-H-TECH'), path, 'low', pa.titleLen + ' characters: "' + pa.title.slice(0, 80) + '".', { confidence: 0.7, severity: 'low' }));
    if (pa.canonical && !U.sameSite(pa.canonical, p.url)) out.push(R.finding(R.get('RAP-H-TECH'), path, 'medium', 'Canonical points off-site: ' + pa.canonical, { confidence: 0.75, severity: 'medium' }));
    if (pa.invalidLd && pa.invalidLd.length) out.push(R.finding(R.get('RAP-H-TECH'), path, 'low', pa.invalidLd.length + ' JSON-LD block(s) failed to parse.', { confidence: 0.88, severity: 'low' }));
    if (pa.metaRefresh) out.push(R.finding(R.get('RAP-H-TECH'), path, 'medium', 'Meta refresh redirect detected.', { confidence: 0.75, severity: 'medium' }));
    if (pa.hreflangs && pa.hreflangs.length) out.push(R.finding(R.get('RAP-H-TECH'), path, 'passed', pa.hreflangs.length + ' hreflang alternate(s).', { confidence: 0.7, severity: 'passed' }));
  ...
  if (perf.pages) {
    out.push(R.finding(R.get('RAP-H-PERFORMANCE'), 'Site', 'info',
