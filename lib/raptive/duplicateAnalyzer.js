...
function analyzeDuplicates(pages, ctx) {
  ...
  if (pairs.length) {
    ...
    out.push(R.finding(R.get('RAP-H-DUPLICATES'), 'Site', status,
    ...
    out.push(R.finding(R.get('RAP-H-DUPLICATES'), 'Site', 'passed',
  ...
  if (dupTitles.length) {
    out.push(R.finding(R.get('RAP-H-DUPLICATES'), 'Site', dupTitles.length >= 3 ? 'high' : 'medium',
