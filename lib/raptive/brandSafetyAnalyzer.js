 ...
 * These are public-content screens, never presented as official Raptive violations unless the
 * category is directly supported by Raptive's brand-safety / Google ad-policy guidance.
 */
...
function analyzeBrandSafety(pages, ctx) {
    ...
    RULES.forEach(rule => {
      ...
      findings.push({
        ...
        evidence: rule.label + ', ' + hits + ' weighted pattern hit(s)' + (headHits ? ' including title/H1' : '')
          ...
          + '. Confidence: ' + confidenceLevel + '. This is a public-content screen, not an official Raptive determination.',
        snippet: ctxSnips[0] || ''
      });
    });
  });
  ...
  if (findings.length) {
    ...
    out.push(R.finding(R.get('RAP-Q-BRAND-SAFETY'), 'Site', status,
      ...
      + ' (' + topCat[0].confidenceLevel + '). Not an official Raptive violation, a public-content screen.',
      { confidence: highCount ? 0.82 : 0.6, severity: status, affected: String(findings.length) }));
  } else {
    out.push(R.finding(R.get('RAP-Q-BRAND-SAFETY'), 'Site', 'passed',
  ...
  findings.forEach(f => {
    out.push(R.finding(R.get('RAP-Q-BRAND-PAGE'), f.page, f.status, f.evidence, {
