...
function analyzeTrust(pages, ctx) {
  ...
  if (have.privacy) {
    ...
    out.push(R.finding(R.get('RAP-Q-PRIVACY'), U.pathOf(have.privacy.url), status, evidence,
      { confidence: have.privacy.confidence / 100, severity: status }));
  } else {
    out.push(R.finding(R.get('RAP-Q-PRIVACY'), 'Site', 'high',
  ...
  if (have.contact && (anyEmail || anyForm || anyPhone)) {
    ...
    out.push(R.finding(R.get('RAP-Q-CONTACT'), U.pathOf(have.contact.url), 'passed',
    ...
    out.push(R.finding(R.get('RAP-Q-CONTACT'), U.pathOf(have.contact.url), 'medium',
  ...
  if (have.about) {
    ...
    out.push(R.finding(R.get('RAP-Q-AUTHOR-TRANSPARENCY'), U.pathOf(have.about.url), sub.words >= 60 ? 'passed' : 'medium',
    ...
    out.push(R.finding(R.get('RAP-Q-AUTHOR-TRANSPARENCY'), 'Site', 'medium',
  ...
  [['terms', 'Terms'], ['disclaimer', 'Disclaimer'], ['cookie', 'Cookie Policy'], ['editorial', 'Editorial Policy']].forEach(pair => {
    ...
    if (e) {
      ...
      out.push({
        id: 'RAP-TRUST-' + k.toUpperCase(), category: 'trust', name: label, status: sub.meaningful ? 'passed' : 'medium',
        ...
        confidence: e.confidence, sourceType: 'quality_signal', sourceUrl: R.SRC.eligible.url, lastVerified: R.VERIFIED, automated: true, weight: 2
      });
    } else {
      out.push({
        id: 'RAP-TRUST-' + k.toUpperCase(), category: 'trust', name: label + ' missing', status: 'low', severity: 'low', page: 'Site',
  ...
  if (articles.length >= 3) {
    ...
    out.push(R.finding(R.get('RAP-Q-AUTHOR-TRANSPARENCY'), 'Site', authorRatio >= 0.4 ? 'passed' : 'low',
  ...
  if (home && home.parse) {
    ...
    out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), U.pathOf(home.url), status,
