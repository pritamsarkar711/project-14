...
function decodeEntities(s) {
  return String(s || '')
    ...
    .replace(/&quot;/gi, '"')
    .replace(/&#39;|&apos;/gi, "'")
    .replace(/&#(\d+);/g, (_, n) => { const c = Number(n); return c ? String.fromCharCode(c) : _; })
    .replace(/&#x([0-9a-f]+);/gi, (_, n) => { const c = parseInt(n, 16); return c ? String.fromCharCode(c) : _; });
}
...
function parseAttrs(str) {
  ...
  const re = /([:@\w.-]+)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/g;
  let m;
  while ((m = re.exec(str || ''))) attrs[m[1].toLowerCase()] = decodeEntities(m[2] != null ? m[2] : (m[3] != null ? m[3] : m[4]));
  return attrs;
}
...
const AD_NETWORKS = [
  ...
  ['Raptive', /raptive\.com|raptive\.cloudfront|grow\.hsvr|grow\.raptive/i],
  ...
  ['Playwire', /playwire\.com/i],
  ['Newor Media', /newormedia\.com/i],
  ['AdSense Auto Ads', /googlesyndication\.com.*auto|adsbygoogle.*auto-ads/i]
];
...
function bodyMainHtml(html) {
  ...
  clone = clone
    ...
    .replace(/<(div|section|ul)[^>]*(id|class)\s*=\s*["'][^"']*(nav|menu|footer|header|sidebar|breadcrumb|comment|widget|cookie|popup|modal|advert)[^"']*["'][^>]*>[\s\S]*?<\/\1>/gi, ' ');
...
function parsePage(result) {
  ...
  const headings = [];
  for (let i = 1; i <= 6; i++) allTags(html, 'h' + i).forEach(t => { headings.push({ tag: 'H' + i, text: textOf(t.inner).slice(0, 180) }); });
  ...
  const images = voidTags(html, 'img').map(a => {
    ...
    return { src, alt: Object.prototype.hasOwnProperty.call(a, 'alt') ? a.alt : null, width: a.width, height: a.height, loading: (a.loading || '').toLowerCase(), srcset: a.srcset || '', format: fmt };
  }).filter(i => i.src);
  ...
  const scriptTags = voidTags(html, 'script').concat((html.match(/<script\b([^>]*)>/gi) || []).map(s => parseAttrs(s.replace(/^<script\b/i, '').replace(/>$/, ''))));
  ...
  const blockingHeadStyles = (head.match(/<link\b[^>]*rel=["']stylesheet["']/gi) || []).filter(s => !/\bmedia\s*=\s*["']print/i.test(s)).length;
  ...
  while ((lm = ldRe.exec(html))) {
    try { const j = JSON.parse(lm[1]); (Array.isArray(j) ? j : [j]).forEach(x => { if (x) jsonLd.push(x); }); }
    catch (e) { invalidLd.push(e.message); }
  }
  const schemaTypes = [];
  jsonLd.forEach(j => { if (j && typeof j === 'object') { const t = j['@type']; if (Array.isArray(t)) t.forEach(x => schemaTypes.push(x)); else if (t) schemaTypes.push(t); } });
  const seenT = {};
  ...
  jsonLd.forEach(j => { if (j.datePublished && !published) published = String(j.datePublished); if (j.dateModified && !modified) modified = String(j.dateModified); });
  if (!published) { const t = html.match(/datetime\s*=\s*["']([^"']+)["']/i); if (t) published = t[1]; }
  if (!modified) { const t = html.match(/itemprop=["']dateModified["'][^>]*content=["']([^"']+)/i); if (t) modified = t[1]; }

  const hasNav = hasLandmark(html, [/<nav\b/i, /role\s*=\s*["']navigation["']/i, /class\s*=\s*["'][^"']*\b(nav|navbar|menu|main-nav)\b/i]);
  const hasFooter = hasLandmark(html, [/<footer\b/i, /role\s*=\s*["']contentinfo["']/i, /id\s*=\s*["']footer["']/i, /class\s*=\s*["'][^"']*\bfooter\b/i]);
  const popups = (html.match(/class\s*=\s*["'][^"']*\b(popup|modal|overlay|interstitial|lightbox|newsletter-box)\b/gi) || []).length
    + (html.match(/id\s*=\s*["'][^"']*(popup|modal|overlay|interstitial|lightbox)[^"']*["']/gi) || []).length;
  const autoplay = (html.match(/<(video|audio)\b[^>]*\bautoplay\b/gi) || []).length
    + (html.match(/<iframe\b[^>]*src=["'][^"']*autoplay/gi) || []).length;
  const hreflangs = linksRel.filter(a => /\balternate\b/i.test(a.rel || '') && a.hreflang).map(a => ({ lang: a.hreflang, href: U.normalizeUrl(a.href || '', base) })).filter(x => x.href);

  const isHttps = /^https:/.test(base);
  const mixedResources = [];
  if (isHttps) { const re = /(?:src|href)\s*=\s*["'](http:\/\/[^"']+)["']/gi; let mm; while ((mm = re.exec(html))) mixedResources.push(mm[1]); }

  const emptyLinks = links.filter(l => l.internal && l.empty).length;
  const buttons = allTags(html, 'button').concat((html.match(/<input\b[^>]*type\s*=\s*["'](submit|button)["'][^>]*>/gi) || []).map(s => ({ attrs: parseAttrs(s), inner: '' })));
  const emptyButtons = buttons.filter(b => { const t = (textOf(b.inner) + (b.attrs.value || '') + (b.attrs['aria-label'] || '')).trim(); return t.length === 0; }).length;
  ...
  const iframes = voidTags(html, 'iframe').concat((html.match(/<iframe\b([^>]*)>/gi) || []).map(s => parseAttrs(s.replace(/^<iframe\b/i, '').replace(/>$/, ''))));
  const adNetworks = detectAdNetworks(html);
  const adScripts = uniqueScripts.filter(s => /pagead|googlesyndication|doubleclick|adsystem|adservice|media\.net|ezoic|monumetric|raptive|adthrive|raptive|grow\.raptive|buysellads|propellerads|taboola|outbrain|mgid|revcontent|playwire|newormedia/i.test(s)).length;
  ...
  const adSlots = (html.match(/ins class=["'][^"']*adsbygoogle|data-ad-client|data-ad-slot|id=["']div-gpt-ad|class=["'][^"']*(adslot|ad-slot|advertisement|ad-placement|grow-in-content)/gi) || []).length;
  const adContainers = (html.match(/class=["'][^"']*\b(ad-|ad_|advert|adsense|ad-slot|adunit)[a-z0-9_-]*\b/gi) || []).length;
  ...
  function detectAnalytics(rawHtml) {
    const h = String(rawHtml || '');
    const ga4Ids = [...new Set((h.match(/\bG-[A-Z0-9]{6,12}\b/g) || []))];
    const uaIds = [...new Set((h.match(/\bUA-\d{4,10}-\d{1,4}\b/g) || []))];
    const gtmIds = [...new Set((h.match(/\bGTM-[A-Z0-9]{4,10}\b/g) || []))];
    const gtag = /gtag\/js|googletagmanager\.com\/gtag|gtag\(/i.test(h);
    const gaJs = /google-analytics\.com\/(analytics|ga)\.js|www\.google-analytics\.com/i.test(h);
    const gtmJs = /googletagmanager\.com\/gtm\.js/i.test(h);
    const head = innerOf(h, 'head');
    const body = innerOf(h, 'body') || h;
    const inHead = /gtag\/js|G-[A-Z0-9]{6,}|GTM-[A-Z0-9]+|google-analytics\.com/i.test(head);
    const inBody = /gtag\/js|G-[A-Z0-9]{6,}|GTM-[A-Z0-9]+|google-analytics\.com/i.test(body);
    const gaScriptCount = (h.match(/gtag\/js|google-analytics\.com\/(analytics|ga)\.js|googletagmanager\.com\/gtm\.js/gi) || []).length;
    return { ga4Ids, uaIds, gtmIds, gtag, gaJs, gtmJs, inHead, inBody, duplicateInstall: ga4Ids.length > 1 || gaScriptCount >= 3 };
  }
  ...
  /* ---- Reader-experience / deep UX signals ---- */
  const stickyElements = (html.match(/class=["'][^"']*\b(sticky|fixed|affix)\b[^"']*["'][^>]*style=[^>]*position/i) || []).length
    + (html.match(/style=["'][^"']*position:\s*(fixed|sticky)/gi) || []).length;
  const fixedOverlays = (html.match(/style=["'][^"']*position:\s*fixed/gi) || []).length
    + (html.match(/class=["'][^"']*\b(sticky-header|floating|fixed-header)\b/gi) || []).length;
  const horizontalOverflowHints = (html.match(/width:\s*\d{4,}px|min-width:\s*\d{4,}px/gi) || []).length;
  const interstitials = (html.match(/\binterstitial\b|\b(exit|welcome|email)-?popup\b/gi) || []).length;
  const autoRefreshAds = (html.match(/googletag\.pubads.*refresh|pubads\.refresh|defineSlot.*refresh|advads.*refresh|refresh.*adunit/i) || []).length;

  /* ---- Audience-first signals ---- */
  const genericIntros = /(in (this|today's|the) (article|post|blog), (we|i)|welcome to our (blog|website)|thank you for (reading|visiting)|in conclusion|to sum up)/i.test(mainText.slice(0, 2000));
  const keywordInHeadings = h1.concat(h2).length ? w.slice(0, 0) /* placeholder */ : false;
  const headingKeywordHits = (h1.concat(h2).join(' ').toLowerCase().match(/\b(best|top|review|guide|how to|tips|vs)\b/g) || []).length;
  const referenceCount = (mainText.match(/\b(according to|source|reference|study|report|research)\b/gi) || []).length;
  const citationMarkers = (html.match(/<a[^>]*rel=["'][^"']*\b(noopener noreferrer)\b|target=["']_blank["'][^>]*href=["']https/i) || []).length;

  /* ---- Trust / author ---- */
  const hasAuthorPage = /\/author\b|\/writers\b|\/team\b/i.test(html);
  const editorial = (mainText.match(/\b(editorial policy|fact[- ]check|correction|disclosure|sources|references)\b/gi) || []).length;
  ...
  result.parse = {
    ...
    blockingHeadScripts, blockingHeadStyles, mediaQueries,
    ...
    adNetworks, adScripts, adIframes, adSlots, adContainers,
    ...
    stickyElements, fixedOverlays, horizontalOverflowHints, interstitials, autoRefreshAds,
    genericIntros, headingKeywordHits, referenceCount, citationMarkers,
    hasAuthorPage, editorial,
    // performance / transport
    hd: result.headers || {},
    ttfb: result.ms || null,
    compression: (result.headers && (result.headers['content-encoding'] || result.headers['transfer-encoding'])) || '',
    cacheHeaders: (result.headers && result.headers['cache-control']) || '',
    cdn: /cloudflare|cloudfront|fastly|akamai|sucuri|cloudfront|stackpath/i.test((result.headers && (result.headers.server + ' ' + (result.headers['via'] || '') + ' ' + (result.headers['cf-ray'] || ''))) || ''),
    htmlSize: result.bytes || html.length,
    analytics
  };
  return result;
}
...
function buildBoilerplateVocab(pages) {
  ...
  parsed.forEach(p => {
    const seen = new Set();
    String(p.parse.visibleText || '').toLowerCase().split(/[^a-z0-9']+/).forEach(w => { if (w.length >= 3 && !seen.has(w)) { seen.add(w); docCounts.set(w, (docCounts.get(w) || 0) + 1); } });
  });
...
module.exports = { parsePage, buildBoilerplateVocab, decodeEntities, textOf, detectAdNetworks, bodyMainHtml };
