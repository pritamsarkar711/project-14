...
function analyzeTrafficSignals(pages, ctx) {
  ...
  if (botLike.length) {
    out.push(R.finding(R.get('RAP-H-BOT-LIKE'), 'Site', 'high',
    ...
    out.push(R.finding(R.get('RAP-H-BOT-LIKE'), 'Site', 'passed',
  ...
  if (incentive.length) {
    out.push(R.finding(R.get('RAP-H-INCENTIVIZED'), 'Site', 'medium',
      'Incentivized-traffic language found on ' + incentive.length + ' page(s): ' + incentive.slice(0, 4).join(', ') + '. Raptive explicitly requires clean, human traffic (not incentivized clicks).',
      { confidence: 0.6, affected: String(incentive.length), severity: 'medium' }));
  } else {
    out.push(R.finding(R.get('RAP-H-INCENTIVIZED'), 'Site', 'passed',
  ...
  if (visibleClaim) {
    out.push(R.finding(R.get('RAP-H-TRAFFIC-CLAIMS'), 'Site', 'info',
      visibleClaim + ' page(s) make publicly visible traffic/size claims. These are self-reported, not verified, and are not treated as proof of Raptive pageviews.',
  ...
  out.push(R.finding(R.get('RAP-H-TRACKING'), 'Site', anyTracking ? 'passed' : 'info',
    anyTracking
  ...
  if (genPages.length) {
    out.push(R.finding(R.get('RAP-H-TRAFFIC-GEN'), 'Site', 'high',
      'Traffic-generation pages detected: ' + genPages.slice(0, 4).join(', ') + '. These are associated with artificial traffic, which Raptive excludes.',
      { confidence: 0.65, affected: String(genPages.length), severity: 'high' }));
  }

  // ---- Manual (Cannot Be Verified) items ----
  [
    ['RAP-MANUAL-PAGEVIEWS', 'Monthly pageviews', 'Actual monthly pageviews (Raptive currently requires 25,000+ in the last 30 days).'],
    ['RAP-MANUAL-SOURCES', 'Traffic sources', 'Actual traffic-source distribution (organic, direct, social, paid, referral).'],
    ['RAP-MANUAL-COUNTRIES', 'Traffic countries', 'Actual country-of-origin distribution and premium-traffic share.'],
    ['RAP-MANUAL-DEMOGRAPHICS', 'Audience demographics', 'Reader demographics (age, gender, interests).'],
    ['RAP-MANUAL-HUMAN', 'Human traffic percentage', 'Share of pageviews/sessions from real humans vs bots/invalid traffic.'],
    ['RAP-MANUAL-HISTORY', 'Traffic quality history', 'Historical traffic authenticity and growth pattern.']
  ].forEach(def => {
    out.push({
      ...
      why: 'Raptive evaluates these data points during review, but they cannot be read from a public URL.',
      fix: 'Provide GA4 evidence for this metric at application time.',
      confidence: 100, sourceType: 'official', sourceUrl: R.SRC.eligible.url, lastVerified: R.VERIFIED,
