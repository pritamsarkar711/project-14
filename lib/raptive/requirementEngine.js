  ...
  R.all().filter(r => !r.automated).forEach(rule => {
    ...
    if ((rule.id === 'RAP-OFFICIAL-COUNTRIES-MID' || rule.id === 'RAP-OFFICIAL-COUNTRIES-HIGH') && user.combined != null && user.provided) {
      ...
      out.push(R.finding(rule, 'Site', met ? 'passed' : 'high',
        'User-provided key-country share: ' + U.round(user.combined, 1) + '% (US ' + (user.us == null ? 'n/a' : user.us) + '%, UK ' + (user.uk == null ? 'n/a' : user.uk) + '%, CA ' + (user.ca == null ? 'n/a' : user.ca) + '%, AU ' + (user.au == null ? 'n/a' : user.au) + '%, NZ ' + (user.nz == null ? 'n/a' : user.nz) + '%). Target at this tier: ' + need + '%+. User-provided, not independently verified.',
