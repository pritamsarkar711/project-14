...
function analyzeUX(pages, ctx) {
  ...
  pages.forEach(p => {
    ...
    if (!vpOk) {
      ...
      out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), path, 'high',
    ...
    if (!pa.hasNav) {
      ...
      out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), path, 'medium',
        'A navigation landmark exists but contains no links.',
        { confidence: 0.7, severity: 'medium' }));
    }
    if (!pa.hasFooter) {
      out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), path, 'low',
    ...
    if (pa.stickyElements || pa.fixedOverlays) {
      ...
      out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), path, pa.fixedOverlays >= 3 ? 'high' : 'medium',
    ...
    if (pa.wordCount > 120 && pa.h1.length !== 1) {
      out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), path, pa.h1.length === 0 ? 'medium' : 'low',
  ...
  if (graph.home && pages.length > 4) {
    ...
    if (orphans.length) {
      out.push(R.finding(R.get('RAP-H-ARCHITECTURE'), 'Site', orphans.length / pages.length >= 0.3 ? 'high' : 'medium',
  ...
  if (overlayUrls.length >= 2) {
    out.push(R.finding(R.get('RAP-APPROVE-READER-EXPERIENCE'), 'Site', 'high',
