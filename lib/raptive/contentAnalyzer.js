...
function analyzeContent(pages, ctx) {
  ...
  targets.forEach(p => {
    ...
    if (skipThinRules(ptype, siteType) && ptype !== 'homepage') {
      if (pa.loremIpsum) {
        out.push(R.finding(R.get('RAP-OFFICIAL-ORIGINAL'), path, 'high',
    ...
    if (uniq.length < 50 || (uniq.length < 80 && pa.wordCount < 220)) {
      ...
      out.push(R.finding(R.get('RAP-OFFICIAL-ORIGINAL'), path, uniq.length < 30 ? 'high' : 'medium',
      ...
      out.push({
        ...
        why: 'Original depth is the core quality signal in Raptive\u2019s audience-first content review.',
        fix: 'Keep the page updated and accurate.',
        confidence: 88,
        sourceType: 'official',
        sourceUrl: R.SRC.eligible.url,
    ...
    if (bp > 0.6) {
      out.push(R.finding(R.get('RAP-H-CONTENT-DEPTH'), path, 'medium',
    ...
    if (riskScore >= 3) {
      ...
      out.push(R.finding(R.get('RAP-H-CONTENT-DEPTH'), path, riskScore >= 5 ? 'high' : 'medium',
    ...
    if (fre != null && fre < 35 && pa.wordCount > 200) {
      out.push({
        ...
        why: 'Heuristic. Not an official Raptive metric.', fix: 'Shorten long sentences where the topic allows.',
  ...
  if (contentPages.length >= 2) {
    ...
    out.push(R.finding(R.get('RAP-H-CONTENT-PORTFOLIO'), 'Site', status,
    ...
    out.push(R.finding(R.get('RAP-OFFICIAL-ORIGINAL'), 'Site', 'medium',
