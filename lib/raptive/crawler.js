...
const UA = 'huvanti-raptive-checker/1.0 (+https://huvanti.com/raptive-eligibility-checker)';
  ...
  async loadAdsTxt() {
    try {
      ...
      if (r.status < 400 && r.text && !/^\s*<(!doctype|html)/i.test(r.text)) {
        this.adsTxt = { present: true, text: r.text.slice(0, 20000), hasRaptive: /raptive\.com|adthrive\.com/i.test(r.text), lineCount: r.text.split(/\r?\n/).filter(l => l.trim() && !l.trim().startsWith('#')).length };
      } else this.adsTxt = { present: false, text: '', hasRaptive: false, lineCount: 0 };
    } catch (e) { this.adsTxt = { present: false, text: '', hasRaptive: false, lineCount: 0 }; }
  }
