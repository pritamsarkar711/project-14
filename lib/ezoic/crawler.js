...
const UA = 'huvanti-ezoic-checker/1.0 (+https://huvanti.com/ezoic-eligibility-checker)';
...
function requestPinned(urlObj, pin, opt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(reqOpts, res => {
      ...
      res.on('data', c => {
        size += c.length;
        if (size > MAX_BYTES) { abort(); return; }
        if (method !== 'HEAD') chunks.push(c);
      });
      res.on('end', () => {
...
async function fetchFollow(rawUrl, opt) {
  opt = opt || {};
  let current;
  try { current = assertPublicUrl(rawUrl); }
  catch (e) { throw e; }
  const hops = [];
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    ...
    if (res.status >= 300 && res.status < 400 && res.headers.location) {
      ...
      catch (e) { throw U.makeError('redirect', 'Invalid redirect location.'); }
...
async function fetchWithRetry(url, opt) {
  ...
  for (let n = 0; n <= retries; n++) {
    try {
      return await fetchFollow(url, opt);
    } catch (e) {
      last = e;
...
function parseRobots(txt) {
  ...
  String(txt || '').split(/\r?\n/).forEach(line => {
    ...
    if ((m = t.match(/^User-agent:\s*(.+)/i))) {
      cur = { agent: m[1].trim().toLowerCase(), disallow: [], allow: [] };
...
function robotsPathAllowed(path, robots) {
  ...
  const rules = [];
  groups.forEach(g => {
    g.allow.forEach(p => rules.push({ allow: true, pattern: p }));
    g.disallow.forEach(p => rules.push({ allow: false, pattern: p }));
  });
  let matched = null;
  rules.forEach(r => {
    if (!r.pattern) return;
    if (path.startsWith(r.pattern) || r.pattern === '/') {
      if (!matched || r.pattern.length >= matched.pattern.length) matched = r;
    }
  });
...
const PRIORITY = [
  ...
  [/privacy/i, 98],
  [/terms|conditions|tos/i, 97],
  [/disclaimer/i, 96],
  [/cookie/i, 95],
  [/\/(blog|article|post|news|guide|tutorial|review)\//i, 70],
  [/\/(product|shop|pricing|plan)/i, 55],
  [/\/(category|collection|topics)/i, 40],
  [/\/$/, 80]
];
function priorityOf(url) {
  for (let i = 0; i < PRIORITY.length; i++) if (PRIORITY[i][0].test(url)) return PRIORITY[i][1];
  return 10;
}
...
class Crawler {
  ...
  enqueue(url, depth) {
    ...
    try {
      const path = U.pathOf(n);
      if (this.robots && !robotsPathAllowed(path, this.robots) && n !== this.start) return;
    } catch (e) { /* continue */ }
  ...
  async loadRobots() {
    ...
    try {
      const r = await this.fetchCached(this.origin + '/robots.txt', { retries: 0, accept: 'text/plain,*/*' });
      if (r.status < 400 && r.text) this.robots = parseRobots(r.text);
    } catch (e) {
      this.robots = parseRobots('');
    }
  }
  ...
  async loadAdsTxt() {
    try {
      ...
      if (r.status < 400 && r.text && !/^\s*<(!doctype|html)/i.test(r.text)) {
        this.adsTxt = {
          present: true,
          text: r.text.slice(0, 20000),
          hasEzoic: /ezoic\.com/i.test(r.text),
          lineCount: r.text.split(/\r?\n/).filter(l => l.trim() && !l.trim().startsWith('#')).length
        };
      } else this.adsTxt = { present: false, text: '', hasEzoic: false, lineCount: 0 };
    } catch (e) {
  ...
  async loadSitemaps() {
    ...
    ['/sitemap.xml', '/sitemap_index.xml', '/sitemap-index.xml', '/wp-sitemap.xml'].forEach(p => {
      candidates.push(this.origin + p);
    });
    const seen = new Set();
    const unique = candidates.filter(u => {
      const n = U.normalizeUrl(u, this.origin);
    ...
    for (const loc of unique) {
      ...
      try {
        ...
        for (const nested of parsed.nested.slice(0, 3)) {
          try {
            const ninfo = await this.fetchCached(nested, { retries: 0 });
            const np = extractSitemapLocs(ninfo.text || '');
            collect.push(...np.urls);
          } catch (e) { /* skip nested */ }
        }
      } catch (e) { /* skip */ }
    }
    ...
    collect.forEach(u => {
      const n = U.normalizeUrl(u, this.origin);
      if (!n || seenU.has(n) || !U.sameSite(n, this.origin) || U.isAsset(n)) return;
      seenU.add(n); uniq.push(n);
    });
  ...
  async crawlOne(item) {
    try {
      ...
      if (ctype && !U.isHtmlCtype(ctype) && !/html/i.test(info.text.slice(0, 200))) {
        this.results.push({
          url: item.url, status: info.status, finalUrl: info.finalUrl, depth: item.depth,
          skipped: true, reason: 'non-html', via: info.via, ms: info.ms, headers: info.headers, hops: info.hops
        });
        return;
      }
      this.results.push({
        url: item.url,
        finalUrl: info.finalUrl || item.url,
        status: info.status,
        depth: item.depth,
      ...
      this.results.push({
        url: item.url, status: 0, depth: item.depth, error: e.message, errorCode: e.code
      });
    }
  }
  ...
  async run() {
    ...
    const loop = async () => {
      while (this.visited.size < this.limit) {
        ...
        this.onProgress({
          stage: 'crawler',
          message: 'Crawled ' + this.visited.size + ' / ' + this.limit + ' pages…',
    ...
    return {
      ...
      reachedLimit: this.queue.length > 0 || this.visited.size >= this.limit,
      sslOk: this.sslOk
    };
  }
}
