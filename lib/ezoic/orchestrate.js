...
const { analyzeMonetization } = require('./monetizationAnalyzer');
const { analyzePolicy } = require('./policyAnalyzer');
...
function crawlConfidence(scan, parsed, ctx) {
  ...
  if (scan.errors && scan.errors.length >= Math.max(3, (scan.pages || []).length * 0.4)) {
    c -= 15; notes.push('A large share of fetch attempts failed.');
  }
...
function analyzeParsed(scan, opt) {
  ...
  pages.forEach(p => {
    if (!p.error && !p.skipped && p.html) {
      try { parsePage(p); } catch (e) { p.error = 'Parse error: ' + e.message; }
    }
  ...
  const ctx = {
    ...
    crawlErrors: (scan.errors || []).length,
    reachedLimit: !!scan.reachedLimit
  };

  onProgress({ stage: 'content', message: 'Analyzing content quality…' });
  let findings = [];
  ...
  onProgress({ stage: 'technical', message: 'Running technical accessibility audit…' });
  findings = findings.concat(analyzeTechnical(pages, ctx));

  onProgress({ stage: 'ux', message: 'Analyzing UX and site architecture…' });
  findings = findings.concat(analyzeUX(parsed, ctx));

  onProgress({ stage: 'trust', message: 'Checking trust and transparency pages…' });
  findings = findings.concat(analyzeTrust(parsed, ctx));

  onProgress({ stage: 'monetization', message: 'Reviewing advertising and monetization signals…' });
  findings = findings.concat(analyzeMonetization(parsed, ctx));

  onProgress({ stage: 'policy', message: 'Screening policy-risk signals…' });
  findings = findings.concat(analyzePolicy(parsed, ctx));

  onProgress({ stage: 'ezoic', message: 'Evaluating documented Ezoic requirements…' });
  findings = findings.concat(evaluateOfficial(parsed, ctx));

  const conf = crawlConfidence(scan, parsed, ctx);
  onProgress({ stage: 'score', message: 'Calculating Ezoic Readiness Score…' });
  const unable = (!parsed.length)
    ? 'The website could not be sufficiently crawled.'
...
async function runAudit(rawUrl, opt) {
  ...
  const crawler = new Crawler(rawUrl, {
    limit: opt.limit || 50,
  ...
  if (scan.challenge && !(scan.pages || []).some(p => p.parse || (p.html && p.html.length > 400 && !/just a moment/i.test(p.html)))) {
    ...
    if (!parsedTry.length) {
      throw U.makeError('challenge', 'The site appears to be behind a Cloudflare or bot challenge and cannot be read automatically.');
    }
  }
