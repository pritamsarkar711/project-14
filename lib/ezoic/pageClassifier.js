...
const IMPORTANT = [
  { key: 'about', label: 'About', patterns: [/about/, /who[- ]we[- ]are/, /our[- ]story/, /\bcompany\b/, /\bteam\b/, /who[- ]we[- ]are/] },
...
function classifyPage(p, importantKeyByUrl) {
  ...
  if (ek) {
    return {
      ...
      blog: 'blog', resources: 'resources', sitemap: 'other'
    }[ek] || 'other';
  }
  ...
  if ((p.parse.schemaTypes || []).indexOf('Article') >= 0 || /\/(blog|news|article|post|story|guide|tutorial)\//i.test(path)) return 'article';
  ...
  if (/\/(category|collection|tag|topics)\b/i.test(path)) return 'category';
  ...
  if (p.parse.wordCount >= 280 && p.parse.headingsCount >= 2) return 'content';
  return 'other';
}

const CONTENT_TYPES = { content: 1, article: 1, blog: 1 };
const UTILITY_TYPES = {
  about: 1, contact: 1, privacy: 1, terms: 1, disclaimer: 1, login: 1,
  search: 1, author: 1, homepage: 1, tool: 1, category: 1, product: 1,
  directory: 1, docs: 1, forum: 1
};
...
function detectImportant(pages) {
  ...
  IMPORTANT.forEach(def => {
    ...
    pages.forEach(p => {
      ...
      let score = 0;
      def.patterns.forEach(pat => {
        if (pat.test(strong)) score += 4;
        else if (pat.test(weak)) score += 1;
      });
      anchors.filter(a => a.href === p.url).forEach(a => {
        def.patterns.forEach(pat => { if (pat.test(String(a.text || '').toLowerCase())) score += 3; });
      });
      if (score >= 3) {
...
function detectSiteType(pages) {
  ...
  const s = {
    blog: 0, news: 0, saas: 0, tools: 0, directory: 0, ecommerce: 0,
    business: 0, portfolio: 0, educational: 0, documentation: 0, forum: 0, other: 0
  };
