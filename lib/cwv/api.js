...
const { fetchViaRelays } = require('./relay');
...
let inflight = 0;
// A connect-phase failure means this server environment has no outbound
// route to the target. Remember it briefly so repeat runs (e.g. the
// Mobile + Desktop pair) answer instantly instead of stalling again.
let egressBlockedUntil = 0;
const EGRESS_CACHE_MS = 60 * 1000;
...
/* ------------------------------------------------------------------ */
/* Core fetch logic (exported for tests; handleFetch wraps it in SSE)  */
/* ------------------------------------------------------------------ */

async function runServerFetch(urlObj, profile, opt) {
  opt = opt || {};
  const onStage = opt.onStage || function () {};
  const signal = opt.signal;
  const relayImpl = opt.relays || fetchViaRelays;
  const directImpl = opt.direct || (async () => {
    const fetcher = createFetcher({});
    return fetcher.fetchUrl(urlObj.href, {
      maxBytes: MAX_HTML_BYTES, timeout: 10000, signal,
  ...
  onStage({ stage: 'validate', message: 'URL validated — checking DNS and connecting…' });
  try {
    home = await directImpl();
  } catch (e) {
    if (signal && signal.aborted) throw err('cancelled', 'The audit was cancelled.');
    const cls = classifyFetchError(e);
    if (cls.code === 'no_egress') egressBlockedUntil = Date.now() + EGRESS_CACHE_MS;
    if (cls.code !== 'no_egress') throw err(cls.code, cls.message);
    // Direct egress unavailable → try public relays from the server itself.
    onStage({ stage: 'relay', message: 'Direct connection unavailable — trying public relays from the server…' });
    let relay;
    try {
      relay = await relayImpl(urlObj.href, { signal });
    } catch (re) {
      throw err('no_egress', 'The server cannot reach external sites from this environment. The page will fall back to a browser-based measurement.');
    }
    viaRelay = relay.relay;
    viaLabel = relay.label;
    home = {
      status: relay.status >= 200 && relay.status < 500 ? relay.status : 200,
      headers: { 'content-type': 'text/html; charset=utf-8' },
      text: relay.html,
      buffer: Buffer.from(relay.html, 'utf8'),
      bytes: relay.html.length,
      rawBytes: relay.html.length,
      truncated: false,
      ms: relay.ms,
      phases: { relayMs: relay.ms },
      protocol: null,
      ip: null,
      hops: [{ url: urlObj.href, status: relay.status || 200, protocol: null }],
      redirects: [],
  ...
  onStage({ stage: 'connect', message: 'Page fetched — rewriting for the measurement sandbox…' });
  ...
  const session = createSession({
    ...
    transportMode: viaRelay ? 'relay' : 'direct',
    viaRelay,
    profile: profile ? {
  ...
  // Relay transport: subresources keep their original absolute URLs and load
  // cross-origin (the browser renders them without CORS); proxying each one
  // through the relay would be too slow. Direct transport: full proxy mode.
  const rewriteSid = viaRelay ? null : session.sid;
  const pageUrl = viaRelay
    ? '/api/cwv-page?sid=' + session.sid + '&mode=relay&u=' + encodeURIComponent(urlObj.href)
    : '/api/cwv-page?sid=' + session.sid;
  ...
  onStage({ stage: 'rewritten', message: 'Page ready — starting the browser measurement…' });
  return {
    sid: session.sid,
    ...
    notes: viaRelay ? ['HTML fetched through the public relay ' + viaLabel + ' (' + Math.round(home.ms) + ' ms) because the server could not reach the site directly. Subresources load cross-origin; sizes and response headers are not observable.'] : []
  };
}
...
async function handleFetch(req, res, body) {
  ...
  const sseHeaders = { 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-cache, no-store', 'Connection': 'keep-alive', 'X-Accel-Buffering': 'no' };
  const ac = new AbortController();
  const timer = setTimeout(() => ac.abort(), FETCH_MS);
  res.on('close', () => ac.abort()); // client disconnected → cancel the scan
  inflight++;
  res.writeHead(200, sseHeaders);

  const stage = state => sendSSE(res, 'progress', state);
  try {
    const result = await runServerFetch(urlObj, profile, {
      signal: ac.signal,
      onStage: stage
    });
...
module.exports = { handleFetch, handlePage, handleProxy, handleMeta, handleAnalyze, runServerFetch };
