...
async function handleFetch(req, res, body) {
  ...
  try {
    // Fast path: a previous connect-phase failure already told us this
    // server environment cannot reach external sites. Answer immediately
    // (streaming proxies must never see a stalled SSE response).
    if (Date.now() < egressBlockedUntil) {
      sendSSE(res, 'error', {
        code: 'no_egress',
        message: 'The server cannot reach external sites from this environment. The page will fall back to a browser-based measurement.'
      });
      return;
    }
    ...
    let home;
    try {
      home = await fetcher.fetchUrl(urlObj.href, {
        maxBytes: MAX_HTML_BYTES, timeout: 10000, signal: ac.signal,
      ...
      throw err(cls.code, cls.message);
    }
