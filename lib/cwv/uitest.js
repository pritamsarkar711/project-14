...
// The fake fetch routes /api/cwv-analyze through the real analyzer (defined
// after the fixture below; referenced inside fetchStub).
...
function makeEnv(mode) {
    ...
    createElement(tag) {
      if (tag === 'iframe') {
        ...
        Object.defineProperty(frame, 'src', {
          set(v) {
            const nonce = nonceFrom(v);
            if (nonce) scheduleStages(nonce);
            else {
              // plain cross-origin URL → direct-iframe limited mode: fire load
              setTimeout(() => {
                const ls = frame.listeners['load'];
                if (typeof ls === 'function') ls({});
                else if (Array.isArray(ls)) ls.forEach(fn => fn({}));
              }, 5);
            }
          },
          get() { return ''; }
  ...
  const JUNK = '{"error":"bad gateway"}';
  function relayResponse(url) {
    const u = String(url);
    if (global.__cwvMode === 'relay-all-fail') return { text: async () => JUNK, status: 200, headers: { get: () => null } };
    // relay-junk: first relay (allorigins raw) returns junk, codetabs works
    if (global.__cwvMode === 'relay-junk' && u.indexOf('api.allorigins.win/raw') >= 0) return { text: async () => JUNK, status: 200, headers: { get: () => null } };
  ...
  const fetchStub = async (url, opt) => {
    if (String(url).indexOf('/api/cwv-fetch') === 0) {
      ...
      if (global.__cwvMode === 'relay-junk' || global.__cwvMode === 'relay-all-fail') {
        // Server transport unavailable → client falls back to relays.
        const text = 'event: error\ndata: ' + JSON.stringify({ code: 'no_egress', message: 'The server cannot reach external sites from this environment.' }) + '\n\n';
        const chunks = [];
        let i = 0;
        while (i < text.length) { const n = 9; chunks.push(text.slice(i, i + n)); i += n; }
        return {
          ok: true, status: 200,
          body: { getReader() { let idx = 0; return { read: async () => idx < chunks.length ? { done: false, value: new Uint8Array(Buffer.from(chunks[idx++], 'utf8')) } : { done: true } }; } },
          json: async () => ({})
    ...
    if (/api\.(allorigins\.win|codetabs\.com|cors\.lol)|corsproxy\.io|cors\.eu\.org/.test(String(url))) {
      return relayResponse(url);
    }
    ...
    if (String(url).indexOf('/api/cwv-analyze') === 0) {
      // Run the REAL analysis on the submitted bundle — the UI test then
      // exercises the true end-to-end pipeline including transport handling.
      try {
        const body = JSON.parse(opt && opt.body ? opt.body : '{}');
        const realReport = analyzeBundle(body.bundle);
        return { ok: true, status: 200, json: async () => realReport };
      } catch (e) {
        return { ok: false, status: 422, json: async () => ({ message: e.message, code: e.code }) };
      }
    }
...
(async function main() {
  ...
  await t('UI: junk response from the first relay is skipped — the next relay completes the audit', async () => {
    const { out } = await runUi('relay-junk');
    assert.ok(out.innerHTML.indexOf('Tool Performance Score') >= 0, 'report rendered via the working relay');
    assert.ok(out.innerHTML.indexOf('The relay returned no readable HTML.') === -1, 'no dead-end failure');
  });

  await t('UI: every relay failing falls back to the limited direct-iframe measurement (honest, real load time)', async () => {
    const { out } = await runUi('relay-all-fail');
    assert.ok(out.innerHTML.indexOf('Limited measurement mode') >= 0, 'limited-mode banner shown');
    assert.ok(out.innerHTML.indexOf('Not Available') >= 0, 'metrics honestly marked Not Available');
    assert.ok(out.innerHTML.indexOf('The relay returned no readable HTML.') === -1, 'no dead-end failure');
  });
