...
function validateBundle(bundle) {
  ...
  // transport whitelist
  if (meta.transport && ['server-proxy', 'browser-direct', 'server-relay', 'direct-iframe'].indexOf(meta.transport) < 0) {
...
function analyzeBundle(bundle, opt) {
  ...
  /* ---- analyzers ---- */
  // In relay/direct-iframe transports the iframe navigation timing is the
  // auditor's own connection, not the target's, never use it as TTFB or as
  // an LCP phase anchor.
  const navIsReal = meta.transport === 'server-proxy' || meta.transport === 'browser-direct';
  const nav = navIsReal ? bundle.nav : null;
  const cls = analyzeCls(vitals);
  const inp = analyzeInp(vitals, bundle.longTasks, bundle.jsFiles);
  const lcp = analyzeLcp(vitals, bundle.resources, bundle.images, bundle.docPhases, nav, bundle.linkHints);
  const fcp = analyzeFcp(vitals, bundle.docPhases, bundle.cssFiles, bundle.jsFiles, bundle.fonts, nav);
  const ttfb = analyzeTtfb(bundle.docPhases, bundle.docHeaders, nav, meta.transport);
