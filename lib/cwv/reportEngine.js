...
function assemble(bundle, R, engineVersion) {
  ...
  const summary = [
    ...
    vitalsRow('tbt', 'TBT', R.longTasks.tbt, 'ms', TH.classify(R.longTasks.tbt, TH.advisory.tbt), true,
      R.longTasks.status === 'unavailable' ? { reason: R.longTasks.note || 'Unable to Measure' }
    ...
    technical: {
      measurement: {
        ...
        transport: (function () {
          ...
          if (t === 'server-proxy') return 'Server-side proxied fetch (subresources loaded through the auditor origin, full Resource Timing + response headers).';
          if (t === 'server-relay') return 'Server-side relay fetch: the HTML was fetched through a public relay (' + (meta.relay || 'unknown') + ') because the server could not reach the site directly. Subresources load cross-origin from the site itself — sizes, response headers and CSS content are not observable.';
          if (t === 'direct-iframe') return 'Direct iframe load (limited): the page was loaded directly in an iframe, but browsers block cross-origin access — no page internals, Core Web Vitals or interactions can be measured from this transport.';
          return 'Browser-direct load (subresources fetched cross-origin by your browser; timing/sizes hidden by the timing-allow-origin policy are marked unavailable).';
        })(),
        ...
        networkThrottle: bundle.profile && bundle.profile.network && bundle.profile.network.label
          ? (meta.transport === 'server-proxy' ? 'Network throttling applied by the auditor proxy: ' + bundle.profile.network.label + '. The throttle shapes proxied responses only.' : 'Network throttling could not be applied in this transport mode — run unthrottled.')
      ...
      limitations: [
        ...
        'In relay transports, cross-origin module scripts (type="module") require CORS headers and may not execute — JavaScript frameworks loaded this way can be degraded.'
      ],
