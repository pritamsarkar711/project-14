...
const { createFetcher, classifyFetchError } = require('./safeFetcher');
t('classifyFetchError: connect-phase failures → no_egress; abort → cancelled; others keep their code', () => {
  assert.strictEqual(classifyFetchError({ code: 'ECONNRESET', connectPhase: true, originalCode: 'ECONNRESET' }).code, 'no_egress');
  assert.strictEqual(classifyFetchError({ code: 'connect_timeout', connectPhase: true }).code, 'no_egress');
  assert.strictEqual(classifyFetchError({ code: 'ENETUNREACH', connectPhase: true, originalCode: 'ENETUNREACH' }).code, 'no_egress');
  // connection reset after TCP connect but with zero response bytes →
  // still an egress-level failure (sandboxed gateway reset, not the site)
  assert.strictEqual(classifyFetchError({ code: 'ECONNRESET', connectPhase: false, noResponse: true, originalCode: 'ECONNRESET' }).code, 'no_egress');
  assert.strictEqual(classifyFetchError({ code: 'ECONNREFUSED', connectPhase: false, noResponse: true }).code, 'no_egress');
  // reset after the server already sent bytes → a real target-site problem
  assert.strictEqual(classifyFetchError({ code: 'ECONNRESET', connectPhase: false, noResponse: false }).code, 'ECONNRESET');
  assert.strictEqual(classifyFetchError({ code: 'aborted', connectPhase: true }).code, 'cancelled');
  assert.strictEqual(classifyFetchError({ code: 'timeout', connectPhase: false }).code, 'unreachable');
  assert.strictEqual(classifyFetchError({ code: 'ENOTFOUND', connectPhase: false }).code, 'ENOTFOUND');
  assert.strictEqual(classifyFetchError({ code: 'network', connectPhase: false }).code, 'network');
});
...
t('Score: null (not 100) when no Core Web Vital was measured', () => {
  const empty = calculateScore({ lcp: null, inp: null, cls: null, ttfb: null, fcp: null, longTasks: { tbt: 0, total: 0 }, waterfall: { requestCount: 0, bytesMeasurable: false }, cache: { status: 'unavailable' }, resources: { compression: { textResources: 0 } }, dom: null });
  assert.strictEqual(empty.value, null);
  assert.ok(empty.note && empty.note.length > 10);
});
...
/* ================= relay fetcher ================= */
const { fetchViaRelays, sniffUsable } = require('./relay');
t('relay: first relay with junk is skipped, first usable HTML wins', async () => {
  const good = '<!doctype html><html><head><title>Relayed</title></head><body><h1>Hello relayed world</h1><p>Content here.</p></body></html>';
  const calls = [];
  const fetchImpl = (u) => {
    calls.push(u);
    if (u.indexOf('allorigins') >= 0) return Promise.resolve({ status: 200, text: async () => '{"error":"bad gateway"}' });
    if (u.indexOf('codetabs') >= 0) return Promise.resolve({ status: 200, text: async () => good });
    return Promise.reject(new Error('nope'));
  };
  const r = await fetchViaRelays('https://example.com/', { fetchImpl, perTimeoutMs: 2000, totalTimeoutMs: 5000 });
  assert.strictEqual(r.relay, 'codetabs');
  assert.strictEqual(r.html, good);
});
t('relay: challenge pages and 4xx are not usable', () => {
  assert.strictEqual(sniffUsable('<html><body>Just a moment...</body></html>', 200), false);
  assert.strictEqual(sniffUsable('{"error":"x"}', 200), false);
  assert.strictEqual(sniffUsable('<html><body>ok</body></html>', 404), false);
  assert.strictEqual(sniffUsable('<!doctype html><html><head><title>x</title></head><body><h1>tiny</h1></body></html>', 200), true);
  assert.strictEqual(sniffUsable('x'.repeat(500), 200), true);
});
t('relay: all relays failing → relay_unreachable', async () => {
  const fetchImpl = () => Promise.reject(new Error('blocked'));
  await assert.rejects(
    () => fetchViaRelays('https://example.com/', { fetchImpl, perTimeoutMs: 300, totalTimeoutMs: 2000 }),
    e => e.code === 'relay_unreachable'
  );
});

/* ================= api runServerFetch (server relay fallback) ================= */
const { runServerFetch } = require('./api');
const { sessionMeta: apiSessionMeta, getSession: apiGetSession } = require('./session');
t('api: no_egress direct → server relay fallback creates a relay-mode session (subresources NOT proxied)', async () => {
  const html = '<!doctype html><html><head><title>Site</title><link rel="stylesheet" href="https://cdn.example/style.css"></head><body><img src="/img/hero.png" alt=""></body></html>';
  const stages = [];
  const result = await runServerFetch(new URL('https://example.com/'), null, {
    direct: async () => { throw Object.assign(new Error('ECONNRESET while requesting example.com'), { code: 'ECONNRESET', connectPhase: false, noResponse: true, originalCode: 'ECONNRESET' }); },
    relays: async () => ({ relay: 'codetabs', label: 'CodeTabs', html, status: 200, ms: 300 }),
    onStage: s => stages.push(s.stage)
  });
  assert.strictEqual(result.mode, 'relay');
  assert.strictEqual(result.relay, 'CodeTabs');
  assert.ok(result.pageUrl.indexOf('mode=relay') >= 0 && result.pageUrl.indexOf('u=' + encodeURIComponent('https://example.com/')) >= 0);
  assert.strictEqual(result.notes.length, 1);
  const meta = apiSessionMeta(result.sid);
  assert.strictEqual(meta.transportMode, 'relay');
  assert.strictEqual(meta.viaRelay, 'codetabs');
  const s = apiGetSession(result.sid);
  assert.ok(s.html.indexOf('/api/cwv-proxy') === -1, 'relay mode must not proxy subresources');
  assert.ok(s.html.indexOf('https://cdn.example/style.css') >= 0, 'subresource URLs kept absolute');
  assert.ok(s.html.indexOf('https://example.com/img/hero.png') >= 0, 'relative subresource resolved to origin');
  assert.ok(s.html.indexOf('/assets/js/cwv/measure.js') >= 0, 'measurement script injected');
  assert.ok(stages.indexOf('relay') >= 0, 'relay stage reported');
});
t('api: direct fetch works → proxy mode session', async () => {
  const html = '<!doctype html><html><head><title>S</title></head><body><h1>Hello</h1><img src="/x.png"></body></html>';
  const result = await runServerFetch(new URL('https://example.com/'), null, {
    direct: async () => ({
      status: 200, headers: { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-cache' },
      text: html, buffer: Buffer.from(html), bytes: html.length, rawBytes: html.length, truncated: false, ms: 300,
      phases: { dnsMs: 10, connectMs: 20, tlsMs: 30, serverMs: 100, ttfbMs: 160, downloadMs: 40, totalMs: 200 },
      protocol: 'h2', ip: '93.184.216.34', hops: [{ url: 'https://example.com/', status: 200, protocol: 'h2' }],
      redirects: [], finalUrl: 'https://example.com/', redirected: false, challenge: false, guard: null
    }),
    relays: async () => { throw new Error('should not be called'); }
  });
  assert.strictEqual(result.mode, 'proxy');
  const s = apiGetSession(result.sid);
  assert.ok(s.html.indexOf('/api/cwv-proxy?sid=') >= 0, 'proxy mode wraps subresources');
});
t('api: direct no_egress + relay failure → no_egress error', async () => {
  await assert.rejects(
    () => runServerFetch(new URL('https://example.com/'), null, {
      direct: async () => { throw Object.assign(new Error('ECONNRESET'), { code: 'ECONNRESET', connectPhase: true, noResponse: true }); },
      relays: async () => { throw Object.assign(new Error('all failed'), { code: 'relay_unreachable' }); }
    }),
    e => e.code === 'no_egress'
  );
});

/* ================= analyzeBundle: relay & direct-iframe transports ================= */
t('analyzeBundle: server-relay bundle: TTFB/cache unavailable, CWV still measured', () => {
  const b = makeBundle();
  b.meta.transport = 'server-relay';
  b.meta.relay = 'CodeTabs';
  delete b.resourceMeta;
  b.docPhases = { relayMs: 500 };
  b.nav = { ttfb: 8, domInteractive: 20, domContentLoaded: 30, load: 40 };
  b.resources.forEach(r => { r.timingAvailable = false; r.duration = 0; r.transferSize = null; r.encodedBodySize = null; r.decodedBodySize = null; });
  const report = analyzeBundle(b);
  assert.strictEqual(report.lab.ttfb.status, 'unavailable');
  assert.strictEqual(report.lab.cache.status, 'unavailable');
  assert.strictEqual(report.lab.lcp.status, 'measured');
  assert.strictEqual(report.lab.inp.status, 'measured');
  assert.ok(report.technical.measurement.transport.indexOf('relay') >= 0);
});
t('analyzeBundle: direct-iframe bundle, everything unavailable, score null, no fabrication', () => {
  const b = makeBundle();
  b.meta.transport = 'direct-iframe';
  b.vitals.lcp = { status: 'unavailable', value: null, entry: null, candidates: [], reason: 'cross-origin' };
  b.vitals.fcp = { status: 'unavailable', value: null, reason: 'cross-origin' };
  b.vitals.cls = { status: 'unavailable', value: null, entries: [], excluded: [], reason: 'cross-origin' };
  b.vitals.inp = { status: 'unavailable', value: null, interactions: [], reason: 'cross-origin' };
  b.vitals.tbt = { status: 'unavailable', value: null, reason: 'cross-origin' };
  b.longTasks = null;
  b.resources = [];
  b.dom = null; b.images = []; b.fonts = []; b.cssFiles = []; b.jsFiles = [];
  b.resourceMeta = null;
  delete b.docPhases;
  b.nav = { ttfb: null, domInteractive: null, domContentLoaded: null, load: 900 };
  b.meta.notes = ['Limited measurement mode, direct iframe load.'];
  const report = analyzeBundle(b);
  assert.strictEqual(report.lab.score.value, null);
  assert.ok(report.lab.score.note && report.lab.score.note.length > 10);
  report.lab.vitals.forEach(v => {
    if (v.key === 'si') return;
    assert.strictEqual(v.value, null, v.key + ' must not be fabricated');
    assert.strictEqual(v.status, 'unavailable', v.key + ' status');
  });
  assert.strictEqual(report.lab.longTasks.status, 'unavailable');
  assert.strictEqual(report.lab.ttfb.status, 'unavailable');
  assert.strictEqual(report.lab.cache.status, 'unavailable');
});
