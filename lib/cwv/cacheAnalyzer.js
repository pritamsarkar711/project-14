...
function analyzeCache(resourceMeta, docHeaders) {
  ...
  if (!resourceMeta || resourceMeta.mode !== 'server-proxy') {
    out.reason = 'Response headers for subresources were not observable in this transport mode (response headers not recorded).';
    if (docHeaders && Object.keys(docHeaders).length) {
