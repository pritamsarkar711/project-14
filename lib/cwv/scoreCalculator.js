...
function calculateScore(ctx) {
  ...
  metric('tbt', 'TBT', 9, ctx.longTasks && ctx.longTasks.status !== 'unavailable' && typeof ctx.longTasks.tbt === 'number' ? ctx.longTasks.tbt : null, 200, 600, true);
  ...
  // Rendering efficiency: long tasks + DOM size heuristic (documented).
  {
    ...
    if (lt && lt.status !== 'unavailable') {
      ...
      note = 'Main-thread tasks not observable — excluded.';
    }
  ...
  // The three Core Web Vitals are the backbone of the score. If none of
  // them were measured, a score would be meaningless — report null instead
  // of scoring an empty bundle.
  const cwvMeasured = ['lcp', 'inp', 'cls'].filter(id => {
    const p = parts.find(x => x.id === id);
    return p && p.score != null;
  }).length;

  let overall = null;
  if (totalWeight && cwvMeasured >= 1) {
    overall = available.reduce((s, p) => s + p.score * p.weight, 0) / totalWeight;
  }
  ...
  return {
    ...
    breakdown: parts,
    note: cwvMeasured === 0 ? 'No Core Web Vital (LCP/INP/CLS) was measured — the score is not reported for this bundle.' : null,
