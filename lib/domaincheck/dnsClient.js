...
function createDnsClient(opts) {
  ...
  async function query(name, type, qopts) {
    qopts = qopts || {};
    const baseKey = name.toLowerCase() + '|' + String(type).toUpperCase() + '|';
    const key = baseKey + (qopts.dnssec ? 'do' : '');
    if (cache.has(key)) return cache.get(key);
    // A DNSSEC capable response contains the ordinary record set too. Reuse it
    // for a later non DNSSEC request instead of repeating the same lookup.
    if (!qopts.dnssec && cache.has(baseKey + 'do')) return cache.get(baseKey + 'do');
