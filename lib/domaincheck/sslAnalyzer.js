...
async function analyzeSsl(host, opt) {
  ...
  // Weak TLS detection, handshake-only probes of legacy versions
  const weakAccepted = [];
  for (const v of ['TLSv1', 'TLSv1.1']) {
    try {
      const probe = transport ? await transport(host, { weakProbe: true, minVersion: v, maxVersion: v }) : await connectOnce(host, 443, { weakProbe: true, minVersion: v, maxVersion: v });
      if (probe.protocol) weakAccepted.push(probe.protocol);
    } catch (e) { /* rejected → fine */ }
  }
