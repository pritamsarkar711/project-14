...
function analyzeTheme(cand, headerRes, parentRes, bundle, probes, opts) {
  ...
  /* ---- Final name ---- */
  const themeName = name || fpName || (slug ? slug : null);
  ...
  const source = detectSource(fp, fields, bundle.siteOrigin);
  ...
  return {
    ...
    version,
    author: fields['Author'] || (fp && fp.author) || null,
    authorUri: fields['Author URI'] || (fp && fp.authorUri) || null,
    themeUri: fields['Theme URI'] || (fp && fp.themeUri) || null,
    ...
    isChild,
    parent,
    preview: opts.screenshot || { available: false },
...
function detectVersion(fields, cand, opts, bundle) {
  ...
  // 3) Not detectable
  return { value: null, source: null, label: 'none', detail: 'Version not publicly detectable — no Version header was readable and no usable asset version parameter was found.' };
}
...
function detectSource(fp, fields, siteOrigin) {
  const uri = (fields && fields['Theme URI']) || (fp && fp.themeUri) || null;
