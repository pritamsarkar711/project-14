...
function assertSafeHostname(hostname) {
  ...
  if (net.isIP(decoded) && isPrivateIp(decoded)) {
    throw makeError('ssrf', 'Private or loopback IP addresses cannot be scanned.');
  }
  return raw;
}
...
async function resolvePublic(hostname) {
  ...
  if (net.isIP(host)) {
    if (isPrivateIp(host)) throw makeError('ssrf', 'Private or loopback IP addresses cannot be scanned.');
...
async function resolveAndPin(urlObj) {
  ...
  // Prefer IPv4: many hosting/egress paths have broken IPv6 routing, which
  // otherwise surfaces as misleading TLS/connection failures.
  const v4 = records.filter(r => r.family === 4);
  const pick = (v4.length ? v4 : records)[0];
  return {
