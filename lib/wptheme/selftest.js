...
/* 31: Blocked homepage but public REST: detected + honest unknown theme */
test('blocked homepage + public REST: WordPress proven, theme honestly unknown', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const bundle = {
    via: 'browser',
    startUrl: 'https://wall.test/', finalUrl: 'https://wall.test/', origin: 'https://wall.test',
    status: 403, headers: {}, headersAvailable: false, homeHtml: '',
    homeBlocked: { status: 403, code: 'blocked' },
    robotsText: 'User-agent: *\nDisallow: /wp-admin/\nDisallow: /wp-content/',
    probes: { rest: { status: 200, text: JSON.stringify({ name: 'Wall', namespaces: ['wp/v2', 'oembed/1.0'] }) }, posts: null, oembed: null },
    candidates: [], themeCssRes: null, parentCssRes: null, mainCss: null,
    screenshot: { attempted: false, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://wall.test/', finalUrl: 'https://wall.test/', status: 403, durationMs: 4000, requests: 6, bytes: 3000, methods: ['URL validation', 'Browser-relayed fetch'], signals: 0, robots: { checked: true, notes: [] }, redirects: [], notes: [] },
    methodsUsed: ['URL validation', 'Browser-relayed fetch', 'WordPress REST API']
  };
  const r = analyzeCollected(bundle);
  assert.strictEqual(r.status, 'detected');
  assert.strictEqual(r.homeBlocked.code, 'blocked');
  assert.ok(!r.theme.found);
  assert.ok(r.theme.why.some(w => /homepage was blocked/i.test(w)));
  assert.ok(r.wordpress.signals.some(sig => sig.key === 'rest_probe'));
});

/* 32: Everything blocked: access failure, never "Not Detected" */
test('everything blocked: throws an access-failure error, never not_detected', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const bundle = {
    via: 'browser',
    startUrl: 'https://wall2.test/', finalUrl: 'https://wall2.test/', origin: 'https://wall2.test',
    status: 403, headers: {}, headersAvailable: false, homeHtml: '',
    homeBlocked: { status: 403, code: 'challenge' },
    robotsText: '', probes: { rest: { status: 403, text: '' }, posts: null, oembed: null },
    candidates: [], themeCssRes: null, parentCssRes: null, mainCss: null,
    screenshot: { attempted: false, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://wall2.test/', status: 403, durationMs: 5000, requests: 7, bytes: 100, methods: [], signals: 0, robots: { checked: false, notes: [] }, redirects: [], notes: [] },
    methodsUsed: []
  };
  let err = null;
  try { analyzeCollected(bundle); } catch (e) { err = e; }
  assert.ok(err, 'must throw');
  assert.strictEqual(err.code, 'challenge');
  assert.ok(/access failure/i.test(err.message));
  assert.ok(!/not wordpress result/i.test(err.message) || /not a .not WordPress. result/i.test(err.message));
});

/* 33: Walled homepage, but the browser asset probe proves WordPress */
test('blocked homepage + browser asset probe: WordPress proven without any fetch', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const bundle = {
    via: 'browser',
    startUrl: 'https://wall3.test/', finalUrl: 'https://wall3.test/', origin: 'https://wall3.test',
    status: 403, headers: {}, headersAvailable: false, homeHtml: '',
    homeBlocked: { status: 403, code: 'blocked' },
    resourceProbe: { loaded: ['https://wall3.test/wp-includes/css/dist/block-library/style.css'] },
    robotsText: '', probes: { rest: null, posts: null, oembed: null },
    candidates: [], themeCssRes: null, parentCssRes: null, mainCss: null,
    screenshot: { attempted: false, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://wall3.test/', status: 403, durationMs: 6000, requests: 9, bytes: 500, methods: [], signals: 0, robots: { checked: false, notes: [] }, redirects: [], notes: [] },
    methodsUsed: []
  };
  const r = analyzeCollected(bundle);
  assert.strictEqual(r.status, 'detected');
  assert.ok(r.wordpress.signals.some(sig => sig.key === 'browser_asset_load'));
  assert.ok(!r.theme.found, 'theme must stay unknown without HTML');
  assert.ok(r.theme.why.some(w => /homepage was blocked/i.test(w)));
});

/* 34: Manual paste: engine runs on user-provided HTML, labelled honestly */
test('manual paste: theme detected from pasted source + labelled', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const pastedHome = wpHome({ origin: 'https://pasted.test', slug: 'astra', bodyClass: 'ast-desktop' });
  const bundle = {
    via: 'browser',
    startUrl: 'https://pasted.test/', finalUrl: 'https://pasted.test/', origin: 'https://pasted.test',
    status: 200, headers: {}, headersAvailable: false, homeHtml: pastedHome,
    manualPaste: true,
    robotsText: '', probes: { rest: null, posts: null, oembed: null },
    candidates: [], // server must rank from the pasted HTML itself
    themeCssRes: { attempted: true, status: 200, text: ASTRA_CSS },
    parentCssRes: null, mainCss: null,
    screenshot: { attempted: false, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://pasted.test/', finalUrl: 'https://pasted.test/', status: 200, durationMs: 800, requests: 1, bytes: pastedHome.length, methods: [], signals: 0, robots: { checked: false, notes: [] }, redirects: [], notes: ['Homepage HTML was pasted manually by the user, the analysis ran on exactly that source.'] },
    methodsUsed: []
  };
  const r = analyzeCollected(bundle);
  assert.strictEqual(r.manualPaste, true);
  assert.ok(r.theme.found && r.theme.name === 'Astra', 'theme ranked from pasted HTML');
  assert.strictEqual(r.theme.version.value, '4.6.12');
  assert.ok(r.scan.notes.some(n => /pasted manually/i.test(n)));
});

/* 35: Blocked homepage, inner page reachable: theme named from inner HTML */
test('blocked homepage + open inner page: theme identified via sitemap discovery', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const innerHtml = wpHome({ origin: 'https://inner.test', slug: 'astra', bodyClass: 'ast-desktop' });
  const bundle = {
    via: 'browser',
    startUrl: 'https://inner.test/', finalUrl: 'https://inner.test/', origin: 'https://inner.test',
    status: 403, headers: {}, headersAvailable: false, homeHtml: innerHtml,
    homeBlocked: { status: 403, code: 'blocked' },
    homeInner: { url: 'https://inner.test/blog/hello-world/' },
    robotsText: '', probes: { rest: null, posts: null, oembed: null },
    candidates: [], // engine must rank from the inner-page HTML
    themeCssRes: { attempted: true, status: 200, text: ASTRA_CSS },
    parentCssRes: null, mainCss: null,
    screenshot: { attempted: true, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://inner.test/', status: 403, durationMs: 12000, requests: 6, bytes: 40000, methods: [], signals: 0, robots: { checked: false, notes: [] }, redirects: [], notes: [] },
    methodsUsed: []
  };
  const r = analyzeCollected(bundle);
  assert.strictEqual(r.status, 'detected');
  assert.ok(r.theme.found && r.theme.name === 'Astra', 'theme named from inner-page HTML');
  assert.strictEqual(r.theme.version.value, '4.6.12');
  assert.ok(r.homeInner && /hello-world/.test(r.homeInner.url));
  assert.ok(r.scan.notes.some(n => /inner page/i.test(n)));
});

/* 36: WordPress.org directory enrichment: names free themes, never fakes the version */
test('wp.org directory: name/author/screenshot from slug, version stays honest', async () => {
  const { analyzeCollected } = require('./orchestrate');
  const bundle = {
    via: 'browser',
    startUrl: 'https://worg.test/', finalUrl: 'https://worg.test/', origin: 'https://worg.test',
    status: 200, headers: {}, headersAvailable: false, homeHtml: wpHome({ origin: 'https://worg.test', slug: 'sydney', generator: 'WordPress 6.8', noVer: true }),
    robotsText: '', probes: { rest: null, posts: null, oembed: null },
    candidates: [], // ranked from homeHtml
    themeCssRes: { attempted: true, status: 403, text: '' }, // style.css blocked
    wporgInfo: { slug: 'sydney', name: 'Sydney', version: '2.4.9', author: 'aThemes', authorUrl: 'https://athemes.com/', homepage: 'https://athemes.com/theme/sydney/', screenshotUrl: 'https://ps.w.org/sydney/screenshot.png', parent: null },
    parentCssRes: null, mainCss: null,
    screenshot: { attempted: true, available: false }, exposureRaw: [],
    scanInfo: { url: 'https://worg.test/', finalUrl: 'https://worg.test/', status: 200, durationMs: 9000, requests: 5, bytes: 30000, methods: [], signals: 0, robots: { checked: false, notes: [] }, redirects: [], notes: [] },
    methodsUsed: []
  };
  const r = analyzeCollected(bundle);
  assert.ok(r.theme.found && r.theme.name === 'Sydney', 'named from the wp.org directory');
  assert.strictEqual(r.theme.author, 'aThemes');
  assert.strictEqual(r.theme.source.label, 'WordPress.org');
  assert.ok(r.theme.preview.available && r.theme.preview.fromDirectory, 'directory screenshot used');
  assert.strictEqual(r.theme.version.label, 'none', 'directory version must NOT be reported as installed');
  assert.ok(/directory currently lists version 2\.4\.9/.test(r.theme.version.detail));
  assert.ok(r.theme.evidence.some(e => e.method === 'WordPress.org directory'));
});
