...
function analyzeCollected(bundle) {
  ...
  const cand = candidates[0] || null;
  if (!cand) {
    return buildUnknownThemeReport({ scanInfo, wp, probes: { rest: bundle.probes.rest, posts: bundle.probes.posts, oembed: bundle.probes.oembed }, mass, via, homeBlocked: bundle.homeBlocked, homeArchived: bundle.homeArchived });
  }
