...
function analyzeCollected(bundle) {
  ...
  if (bundle.resourceProbe) probesFull.resourceProbe = bundle.resourceProbe;
  if (bundle.manualPaste) scanInfo.notes.push('Homepage source was provided manually (paste), no automated homepage fetch was used.');
  if (bundle.homeInner) scanInfo.notes.push('Analysis HTML came from a public inner page (' + bundle.homeInner.url + ') because the homepage was blocked.');

  const wp = analyzePage({ url: bundle.startUrl, finalUrl: bundle.finalUrl, html, headers: via === 'browser' ? {} : bundle.headers }, { robots: { text: bundle.robotsText }, rest: probesFull.rest, resourceProbe: probesFull.resourceProbe });
  ...
  /* Live homepage blocked (and no archived copy worked): if the remaining
     evidence still proves WordPress, say so honestly with an unknown theme.
     If it does not, this is an access failure: NEVER "not WordPress". */
  if (bundle.homeBlocked && !html.length && wp.score < 15) {
    const isChallenge = bundle.homeBlocked.code === 'challenge';
    const e = U.makeError(isChallenge ? 'challenge' : 'blocked',
      (isChallenge
        ? 'The live homepage is protected by a bot challenge that defeats every transport (direct fetch + public relays)'
        : 'The live homepage was refused on every transport (direct + public relays; HTTP ' + bundle.homeBlocked.status + ')') +
      (bundle.probes.rest && bundle.probes.rest.status ? (bundle.probes.rest.status >= 400 ? '; the REST API also returned HTTP ' + bundle.probes.rest.status : '') : '; the REST API was unreachable') + '. ' +
      'WordPress status cannot be determined, this is an access failure, not a “not WordPress” result.');
    e.scan = scanInfo;
    throw e;
  }
  ...
  if (wp.status === 'not_detected' && (!bundle.homeBlocked || html.length)) {
  ...
  const cand = candidates[0] || null;
  if (!cand) {
    return buildUnknownThemeReport({ scanInfo, wp, probes: { rest: bundle.probes.rest, posts: bundle.probes.posts, oembed: bundle.probes.oembed }, mass, via, homeBlocked: bundle.homeBlocked, homeArchived: bundle.homeArchived, homeInner: bundle.homeInner, manualPaste: bundle.manualPaste });
  }
  ...
  const theme = analyzeTheme(cand, headerRes, parentRes, restBundle, {
    ...
    wpVersion,
    wporg: bundle.wporgInfo || null,
    homeInner: bundle.homeInner || null,
  ...
  const versionStatus = buildVersionStatus(theme, bundle.wporgInfo);
  ...
  if (bundle.homeBlocked && !html.length) scanInfo.notes.push('The live homepage was blocked, so this verdict rests on other public endpoints (REST/robots) and the active theme could not be identified.');
  const report = {
    status: wp.status === 'detected' ? 'detected' : 'likely',
    statusLabel,
    via,
    homeBlocked: bundle.homeBlocked || null,
    homeArchived: bundle.homeArchived || null,
    homeInner: bundle.homeInner || null,
    manualPaste: !!bundle.manualPaste,
...
function sanitizeBundle(bundle) {
  ...
  out.homeBlocked = out.homeBlocked ? { status: Number(out.homeBlocked.status) || 0, code: String(out.homeBlocked.code || '').slice(0, 30) } : null;
  out.homeArchived = out.homeArchived ? { timestamp: String(out.homeArchived.timestamp || '').slice(0, 20) } : null;
  out.manualPaste = !!out.manualPaste;
  out.homeInner = out.homeInner ? { url: String(out.homeInner.url || '').slice(0, 300) } : null;
  out.resourceProbe = out.resourceProbe && Array.isArray(out.resourceProbe.loaded) && out.resourceProbe.loaded.length
    ? { loaded: out.resourceProbe.loaded.slice(0, 3).map(x => String(x).slice(0, 200)) } : null;
  const w = out.wporgInfo;
  out.wporgInfo = w && typeof w === 'object' && w.name ? {
    slug: String(w.slug || '').slice(0, 80), name: String(w.name).slice(0, 120),
    version: w.version ? String(w.version).slice(0, 30) : null,
    author: w.author ? String(w.author).slice(0, 120) : null,
    authorUrl: w.authorUrl ? String(w.authorUrl).slice(0, 300) : null,
    homepage: w.homepage ? String(w.homepage).slice(0, 300) : null,
    screenshotUrl: w.screenshotUrl ? String(w.screenshotUrl).slice(0, 400) : null,
    parent: w.parent && w.parent.slug ? { slug: String(w.parent.slug).slice(0, 80), name: w.parent.name ? String(w.parent.name).slice(0, 120) : null } : null
  } : null;
...
function buildVersionStatus(theme, wporg) {
  ...
  let fp = FP.findBySlug(theme.slug);
  let latestKnown = fp && fp.latestKnown ? Object.assign({ source: 'bundled fingerprint dataset' }, fp.latestKnown) : null;
  if (!latestKnown && wporg && wporg.version && theme.slug && wporg.slug === theme.slug.toLowerCase()) {
    latestKnown = { version: wporg.version, asOf: 'today', source: 'public WordPress.org theme directory' };
  }
  if (!latestKnown) {
  ...
  const c = U.cmpVersion(theme.version.value, latestKnown.version);
  const ds = 'Reference: ' + latestKnown.source + ' (newest recorded release ' + latestKnown.version + ', as of ' + latestKnown.asOf + '). This reference may lag behind reality, verify with the vendor.';
  if (c === 0) return { label: 'Current according to the available dataset', detail: 'The detected version matches the newest release in the reference dataset. ' + ds };
  if (c < 0) return { label: 'Older version detected', detail: 'The detected version (' + theme.version.value + ') is older than the newest release in the reference dataset (' + latestKnown.version + '). ' + ds + ' No vulnerability claims are made from this comparison.' };
  return { label: 'Newer than the dataset', detail: 'The detected version (' + theme.version.value + ') is newer than the reference dataset (' + latestKnown.version + '), the dataset is likely stale. ' + ds };
}
...
function buildUnknownThemeReport(ctx) {
  ...
  scanInfo.durationMs = scanInfo.durationMs || 0;
  scanInfo.signals = ctx.wp.signals.length;
  if (ctx.homeBlocked && scanInfo.notes.indexOf('The live homepage was blocked, so no theme asset paths could be read from its HTML.') < 0) {
    scanInfo.notes.push('The live homepage was blocked (' + (ctx.homeBlocked.code === 'challenge' ? 'bot challenge' : 'HTTP ' + ctx.homeBlocked.status) + ').');
  }
  const attempts = [
  ...
  const why = [];
  if (ctx.homeBlocked) why.push('The live homepage was blocked (' + (ctx.homeBlocked.code === 'challenge' ? 'bot challenge' : 'HTTP ' + ctx.homeBlocked.status) + '), so no theme asset paths could be read from its HTML.');
  if (ctx.homeArchived) why.push('An archived homepage snapshot was used for discovery, the live theme may have changed since the snapshot.');
  ...
  return {
    ...
    via: ctx.via || 'server',
    homeBlocked: ctx.homeBlocked || null,
    homeArchived: ctx.homeArchived || null,
    homeInner: ctx.homeInner || null,
    manualPaste: !!ctx.manualPaste,
    scannedAt: new Date().toISOString(),
    wordpress: {
