...
function requestPinned(urlObj, pin, opt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', e => {
      ...
      if (/cert|ssl|tls|unable to verify|EPROTO|DEPTH_ZERO/.test(m)) reject(U.makeError('ssl', 'SSL/TLS connection failed. Check the certificate and HTTPS setup.', e));
      else if (/ECONNREFUSED|EHOSTUNREACH|ENETUNREACH/.test(m)) reject(U.makeError('unreachable', 'Could not reach the server (connection refused or unreachable).', e));
