 ...
 * Does NOT penalize a site merely for containing ads: Mediavine evaluates reader experience
...
function analyzeAdvertising(pages, ctx) {
  ...
  const hasMediavine = netList.some(n => /mediavine|grow\.mediavine/i.test(n));
  const competing = netList.filter(n => !/mediavine|grow\.mediavine/i.test(n));

  if (netList.length) {
    out.push(R.finding(R.get('MV-H-AD-DENSITY'), 'Site', 'info',
      'Detected ad network(s): ' + netList.join(', ') + ' on ' + adPages + ' page(s). Existing advertising is not automatically a problem. If you join Mediavine, non-Mediavine tags must be removed.',
      { confidence: 0.8, severity: 'info' }));
  } else {
    out.push(R.finding(R.get('MV-H-AD-DENSITY'), 'Site', 'passed',
  ...
  if (heavy.length) {
    out.push(R.finding(R.get('MV-H-AD-DENSITY'), 'Site', 'medium',
  ...
  out.push(R.finding(R.get('MV-H-ADSTXT'), 'Site', adsTxt.present ? 'passed' : 'low',
    adsTxt.present ? 'ads.txt is present (' + (adsTxt.lineCount || 0) + ' non-comment line(s))' + (adsTxt.hasMediavine ? ' and already lists mediavine.com' : ' without Mediavine lines yet') + '.'
      : 'No ads.txt found at /ads.txt. Mediavine will manage ads.txt after integration; its absence before joining is not a quality failure.',
